...
const TeacherSignUp = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      const [deptRes, batchRes, secRes] = await Promise.all([
  ...
  return (
    <div className="container mx-auto  px-4 pt-2 pb-1 flex justify-center">
      ...
      <div className="bg-white max-w-[800px]  sm:w-[90%] md:w-[508px] p-6 md:p-8 rounded-xl shadow-md">
        <p className="text-xl font-bold text-center mb-2 text-gray-700">Teacher Registration</p>
        ...
        <p className="d-flex justify-center w-full m-0 mb-1 font-size-20px text-gray-600 font mt-4">
        ...
        <span className="d-flex justify-center w-full font-size-20px text-gray-600 fs ">
          ...
          <Link to="/facultyLogin" className="text-blue-500  font-size-21px pl-1 font_use hover:underline">
            Register Here
          </Link>
        </span>
      </div>
