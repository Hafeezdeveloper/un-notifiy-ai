...
const EditAnnoucenment = () => {
    ...
    const fetch = async () => {
        setIsLoading(true);
        const response = await GetApi<any>(`/announcements/${id}`);
        if (response?.data?.success) {
            setFormData({
                description: response?.data.document.description,
                role: response?.data.document.role,
            });
            setIsLoading(false);
        } else {
            toast.error('error');
    ...
    const sendAnnouncement = async (e: React.FormEvent) => {
        ...
        try {
            setIsLoading(true);
            const response = await PutApi<any>('/announcements', formData, id);
            ...
            <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl">
                <div className="p-6 md:p-8">
                    <div className="mb-6 flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-blue-900">Edit Announcement</h2>
                            <p className="text-sm text-gray-600">Dashboard / Edit Announcements</p>
                        </div>
                        <button
...
export default EditAnnoucenment;
